...
export interface RelayAddress { host: string; port: number; baseURL: string }

interface Fingerprint { name: string; headers: Record<string, string> }

const CLAUDE: Fingerprint = {
  name: 'claude-cli',
  headers: {
...
function candidateList(model: string | undefined): Fingerprint[] {
  return model?.toLowerCase().includes('claude')
    ? [CLAUDE, CODEX_RS, CODEX_SDK, OPENAI_SDK]
    : [CODEX_RS, CODEX_SDK, OPENAI_SDK, CLAUDE]
}

function fingerprintHeaders(fp: Fingerprint): Record<string, string> {
  const headers = { ...fp.headers }
  if (fp === CODEX_RS) headers.session_id = randomUUID()
  return headers
}

function requestBody(req: IncomingMessage, maxBytes: number): Promise<Buffer> {
...
function normalizedUpstream(value: string): URL {
  ...
  if (url.protocol !== 'https:' && url.protocol !== 'http:') throw new TypeError('upstream must use http or https')
  url.pathname = url.pathname.replace(/\/$/, '')
  return url
}

function clientHeaders(source: IncomingHttpHeaders, authorization: string): Headers {
...
export class AirOuterRelay {
  ...
  readonly #cache = new Map<string, Fingerprint>()
  #server?: Server
  ...
  async start(): Promise<RelayAddress> {
    if (this.#server) throw new Error('relay is already started')
    ...
    if (address === null || typeof address === 'string') throw new Error('relay did not obtain a TCP address')
    ...
    try {
      ...
      try {
        for (const [index, fp] of candidates.entries()) {
          ...
          if (this.#options.verbose) this.#options.logger?.debug?.(`air-outer: ${model ?? 'unknown'} try ${fp.name}`)
          ...
          if (upstream.status < 400) {
            if (model) this.#cache.set(model, fp)
      ...
      if (!res.headersSent) this.#jsonError(res, code, error instanceof Error ? error.message : String(error))
      else res.destroy()
    }
  }

  #jsonError(res: ServerResponse, status: number, message: string): void {
